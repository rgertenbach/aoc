...
def propose_move(current: State, direction: str, width: int, height: int) -> State | None:
  if direction == '^':
    if current.pos.y <= 0:
      return None
    return State(Point(current.pos.x, current.pos.y - 1), current.cycle + 1)
  if direction == 'v':
    if current.pos.y == height - 1 and current.pos.x != width - 1:
      return None
    return State(Point(current.pos.x, current.pos.y + 1), current.cycle + 1)
  if direction == '<':
    if current.pos.x == 0:
      return None
    return State(Point(current.pos.x - 1, current.pos.y), current.cycle + 1)
  if direction == '>':
    if current.pos.x == width - 1 or current.pos.y == -1:
      return None
    return State(Point(current.pos.x + 1, current.pos.y), current.cycle + 1)
  if direction == '.':
    return State(current.pos, current.cycle + 1)
