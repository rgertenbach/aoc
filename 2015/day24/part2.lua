...
local weight_per_group = total_weight // 4
f:close()
...
local function fits(grp1, candidates)
  for _, grp2 in ipairs(candidates) do
    if not intersect(grp1, grp2) then
      for _, grp3 in ipairs(candidates) do
        if not intersect(grp1, grp3) and not intersect(grp2, grp3) then
          return true
        end
      end
    end
